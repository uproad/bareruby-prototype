...
| `array_of_objects.rb` | A fixed-capacity array whose elements are objects — a class the program wrote and a peripheral class alike. An array created into holds its objects; one handed objects that already exist names them, so assignment into an element shares as Ruby does. Matches real Ruby |
