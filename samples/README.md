...
| `arena.rb` | `arena(N)` as a form: nested blocks as release points, `arena` without a size, methods that allocate without being handed a region, growing arrays (`Arena::Array`, `[]`, `[]=` past the end, `<<`), `::Array` for the fixed-capacity one, and a region left by an exception |
