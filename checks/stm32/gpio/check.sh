#!/usr/bin/env bash
# Hold the STM32 GPIO binding to fixed, reviewed answers under Renode.
set -uo pipefail
...
RUBY=${RUBY:-ruby}
TARGET=${1:-f446}
HERE=checks/stm32/gpio
WORK=.bareruby/checks/stm32/gpio
...
ATTACH='machine LoadPlatformDescriptionFromString "button: Miscellaneous.Button @ gpioPortC 13 { invert: true; -> gpioPortC@13 }"'
...
# The input checks need somebody pressing the board's B1 button mid-run, and register
# and LED-state observations have to be said before quit — `bareruby emulate` runs
# Renode with none of that. So the emulate verb builds with a stub standing in for
# Renode (it satisfies the verb by leaving an empty UART capture), and the one real
# Renode run below is ours, on the transformed script.
...
mapfile -t checks < <("$RUBY" -ryaml -e '
  YAML.safe_load_file(ARGV[0])["checks"].each do |check|
    puts [check.fetch("name"), check.fetch("observe", ""),
          check.fetch("press", "")].join("|")
  end
' "$HERE/checks.yml")

for check in "${checks[@]}"; do
    IFS='|' read -r name observe press <<<"$check"
    ...
    # The generated script, with the button made real: attached right after the
    # platform loads, pressed between two slices of the run when the check asks for
    # it, and any observation said before quit.
    probe=""
    [ -n "$observe" ] && probe="$HERE/$observe"
    awk -v attach="$ATTACH" -v probe="$probe" -v press="$press" '
        BEGIN {
            if (probe != "") { while ((getline line < probe) > 0) { held = held line ORS } }
        }
        /^quit$/ { printf "%s", held }
        press == "true" && /^emulation RunFor / {
            print "emulation RunFor \"0:00:00.2\""
            print "sysbus.gpioPortC.button Press"
        }
        { print }
...
echo "stm32 gpio: $passed/$((passed + failed)) checks passed on $TARGET"
[ "$failed" -eq 0 ]
