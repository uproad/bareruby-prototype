...
mapfile -t checks < <("$RUBY" -ryaml -e '
  YAML.safe_load_file(ARGV[0])["checks"].each do |check|
    puts [check.fetch("name"), check.fetch("input", ""),
          ...
          check.fetch("capture", ""), check.fetch("pattern", "")].join("|")
  end
' "$HERE/checks.yml")

for check in "${checks[@]}"; do
    IFS='|' read -r name input observe input_bytes feed chunk_size capture pattern <<<"$check"
    ...
    arguments=()
    feed_path=""
    # The generated bytes repeat 1..251 so a lost or duplicated byte breaks the cadence;
    # pattern "full" walks 0..255 instead, so 0x00 and 0xFF themselves cross the queue.
    if [ -n "$input_bytes" ]; then
        input="$kept/input.bin"
        "$RUBY" -e '
          count = Integer(ARGV.fetch(0))
          full = ARGV.fetch(2) == "full"
          File.binwrite(ARGV.fetch(1),
                        (0...count).map { |i| full ? i % 256 : i % 251 + 1 }.pack("C*"))
        ' "$input_bytes" "$input" "$pattern"
    fi
