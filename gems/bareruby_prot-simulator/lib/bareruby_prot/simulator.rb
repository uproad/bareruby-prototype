...
module BareRubyProt
  # A host build, run where there is no board: the instructions are interpreted here, and
  # every call a program makes into a peripheral lands on an object instead of on a stub
  # that prints what it was asked.
  #
  # **What that buys is a board that is still there afterwards.** A trace says a pin was
  ...
  # `Run#board`, and [`API.md`](../../API.md) is what it answers.
  module Simulator
