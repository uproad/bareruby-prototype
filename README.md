...
- **The STM32Cube binding** — a user-owned NUCLEO-F446RE CubeMX project, kept under
  `.tools/stm32cube/`, owns clock, pin, startup, HAL initialization and link
  configuration. Pass
...
└── stm32cube/
    ├── F446_Sample/                    # the CubeMX project, generated by the desk
    ├── stm32cubeide_2.2.0/
    ├── workspace/                      # CubeIDE's, made on demand
    └── configuration/
```
...
The NUCLEO-F446RE target uses a CubeMX/CubeIDE project the desk generates for itself.
CubeMX owns clocks, pins, HAL initialization, startup and linker scripts; pass 12
supplies C++ through the HAL.
...
CubeIDE itself is looked for under `.tools/stm32cube/stm32cubeide_*` first, then in `PATH`
and under the normal `/opt/st` installation tree, which is where ST's installer puts it;
`STM32CUBEIDE=/path/to/headless-build.sh` overrides all of that. The toolchain copies in
only the translation units this program reached for, replaces nothing but the files it
owns, and brings the linked ELF back to `build/<composition>/bareruby_program.elf` so that
flashing needs to know nothing about how the Cube project arranges its output.
